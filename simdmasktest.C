...
bool vectorsEqual(const SIMDVec<T, SIMD_WIDTH> &a, const SIMDVec<T, SIMD_WIDTH> &b) {
    return test_all_ones(cmpeq(a, b));
}
bool vectorsEqual(const SIMDVec<SIMDFloat, 16> &a, const SIMDVec<SIMDFloat, 16> &b) {
...
bool vectorsEqual(const __m128 a, const __m128 b) {
    return _mm_movemask_epi8(_mm_cmpeq_epi8(_mm_castps_si128(a), _mm_castps_si128(b)))==0xffff;
}
#endif

#ifdef __AVX2__
bool vectorsEqual(const __m256i a, const __m256i b) {
    return ((unsigned int) _mm256_movemask_epi8(_mm256_cmpeq_epi8(a, b)))==0xffffffff;
}

bool vectorsEqual(const __m256 a, const __m256 b) {
    return ((unsigned int) _mm256_movemask_epi8(_mm256_cmpeq_epi8(_mm256_castps_si256(a), _mm256_castps_si256(b))))==0xffffffff;
}
#else
#ifdef __AVX__
bool vectorsEqual(__m256i a, __m256i b) {
    return test_all_ones(cmpeq(SIMDVec<SIMDByte,32>(a), SIMDVec<SIMDByte,32>(b)));
}

bool vectorsEqual(__m256 a, __m256 b) {
    return test_all_ones(cmpeq(SIMDVec<SIMDByte,32>(_mm256_castps_si256(a)), SIMDVec<SIMDByte,32>(_mm256_castps_si256(b))));
}
#endif
#endif
...
bool vectorsEqual(const __m512i a, const __m512i b) {
    //return _mm512_cmpeq_epi8_mask(a, b)==0xffffffffffffffff;//test_all_zeros(cmpeq(a, b));
    return _mm512_cmpeq_epi64_mask(a, b)==0xff;
    //return (vectorsEqual(SIMDVec<SIMDByte,64>(a).lo(), SIMDVec<SIMDByte,64>(b).lo())) && (vectorsEqual(SIMDVec<SIMDByte,64>(a).hi(), SIMDVec<SIMDByte,64>(b).hi()));
}

bool vectorsEqual(const __m512 a, const __m512 b) {
    //return _mm512_cmpeq_epi8_mask(_mm512_castps_si512(a), _mm512_castps_si512(b))==0xffffffffffffffff;//test_all_zeros(cmpeq(a, b));
    return (vectorsEqual(SIMDVec<SIMDFloat,64>(a).lo(), SIMDVec<SIMDFloat,64>(b).lo())) && (vectorsEqual(SIMDVec<SIMDFloat,64>(a).hi(), SIMDVec<SIMDFloat,64>(b).hi()));
}
#endif
...
//Test any function with two operands ("NAME" is the name of the function, e.g. "add" or "and")
#define TEST_FUNCTION(NAME, OP, TYPE, SIMD_WIDTH) { \
unsigned int csr0, csr1, csr2; \
SIMDVec<TYPE, SIMD_WIDTH> src=setzero<TYPE, SIMD_WIDTH>(), a=getRandomVector<TYPE, SIMD_WIDTH>(), b=getRandomVector<TYPE, SIMD_WIDTH>(), res1, res2; \
/* First test */ \
...
if(!vectorsEqual(res1, setzero<TYPE, SIMD_WIDTH>()) || getcsr()!=csr0) \
{ printf("Problem with %s for %s test 2, csr=%x, csr0=%x \na=", #NAME, #TYPE, getcsr(), csr0); print("%e ", a); printf("\nres1="); print("%e ", res1); printf("\nzeros="); print("%e ", setzero<TYPE, SIMD_WIDTH>()); puts("");} \
setcsr(csr0); \
/* Third test */ \
unsigned int i; \
...
{ printf("Problem with %s for %s test 3, csr1=%x, csr2=%x result 1=", #NAME, #TYPE, csr1, csr2); \
for(i=0; i<SIMD_WIDTH/sizeof(TYPE); i++) { printf(FORMAT, result1[i]); } printf(" result2="); \
for(i=0; i<SIMD_WIDTH/sizeof(TYPE); i++) { printf(FORMAT, result2[i]); } printf(" mask=%lx=%lx\n", mask, (uinttest_t) rand_mask); } \
 else { if (PRINT_PASS) printf("Pass test 2 %s for %s \n", #NAME, #TYPE); } \
} \
setcsr(csr0); \
}
...
unsigned int csr0; \
csr0=getcsr(); \
SIMDVec<TYPE, SIMD_WIDTH> a=getRandomVector<TYPE, SIMD_WIDTH>(), b=getRandomVector<TYPE, SIMD_WIDTH>(); \
...
{ printf("Problem with %s for %s test 2, csr=%x, csr0=%x \na=", #NAME, #TYPE, getcsr(), csr0); print("%e ", a); printf("\nres1=%lx", (uinttest_t) res1); printf("\n");} \
setcsr(csr0); \
/* Third test */ \
unsigned int i; \
if(strcmp(#OP, "dummy")!=0) { \
...
if(result1!=result2) \
{ printf("Problem with %s for %s test 3, result 1=%lx, result2=%lx  mask=%lx=%lx\narg1=", #NAME, #TYPE, result1, result2, mask, (uinttest_t) rand_mask); \
for(i=0; i<SIMD_WIDTH/sizeof(TYPE); i++) { printf(FORMAT, arg1[i]); } printf("\narg2="); \
for(i=0; i<SIMD_WIDTH/sizeof(TYPE); i++) { printf(FORMAT, arg2[i]); } puts(""); } \
 else { if (PRINT_PASS) printf("Pass test 2 %s for %s \n", #NAME, #TYPE); } \
} \
setcsr(csr0); \
}
...
if(result1!=result2) \
{ printf("Problem with mask_test_all_ %s for %s test 3, result 1=%i, result 2=%i", #NAME, #TYPE, result1, result2); printf(" mask=%lx=%lx\n arg=", mask, (uinttest_t) rand_mask); \
for(i=0; i<SIMD_WIDTH/sizeof(TYPE); i++) { printf(FORMAT, arg[i]); } puts(""); } \
 else { if (PRINT_PASS) printf("Pass test 2 %s for %s \n", #NAME, #TYPE); } \
}
...
if(!vectorsEqual(res1, setzero<TYPE, SIMD_WIDTH>()) || getcsr()!=csr0) \
{ printf("Problem with %s for %s, csr=%x, csr0=%x \n", "load", #TYPE, getcsr(), csr0); printf("res1="); print(FORMAT, res1); printf("\n");} \
setcsr(csr0); \
/* Third test */ \
...
csr1=getcsr(); \
setcsr(csr0); \
res2=mask_ifelse(krand, load<SIMD_WIDTH, TYPE>(buffer), setzero<TYPE, SIMD_WIDTH>()); \
csr2=getcsr(); \
if(!vectorsEqual(res1, res2) || csr1!=csr2) \
{ printf("Problem with %s for %s, csr1=%x, csr2=%x \n", "load", #TYPE, csr1, csr2); printf("res1="); print(FORMAT, res1); printf("\nres2="); print(FORMAT, res2); printf("\nres1="); print("%x ", reinterpret<SIMDInt>(res1)); printf("\nres2="); print("%x ", reinterpret<SIMDInt>(res2)); puts(""); } \
 else { if (PRINT_PASS) printf("Pass %s for %s \n", "load", #TYPE); }	\
...
if(memcmp(buffer1, buffer2, SIMD_WIDTH)!=0 || csr1!=csr2) /* TODO problem with flags */ \
{ printf("Problem with %s for %s, csr1=%x, csr2=%x \n", "store", #TYPE, csr1, csr2); } \
 else { if (PRINT_PASS) printf("Pass %s for %s \n", "store", #TYPE); }	\
SIMDMask<TYPE, SIMD_WIDTH> kzero; \
setcsr(csr0); \
...
void test_cvts() {
    ...
    SIMDVec<Tout, SIMD_WIDTH> res1, res2, src=getRandomVector<Tout, SIMD_WIDTH>();
    /* First test */
    ...
    print("%e ", res1); printf("\nres2="); print("%e ", res2); puts("");*/
    if(!vectorsEqual(res1, res2) || csr1!=csr2) /* TODO problem with flags */
    { printf("Problem with %s for %s test 1, csr1=%x, csr2=%x \na=", "cvts", __PRETTY_FUNCTION__, csr1, csr2); print("%e ", a); printf("\nres1="); print("%e ", res1); printf("\nres2="); print("%e ", res2); printf("\nres1="); print("%x ", reinterpret<SIMDInt>(res1)); printf("\nres2="); print("%x ", reinterpret<SIMDInt>(res2)); puts(""); }
    else { if (PRINT_PASS) printf("Pass cvts for Tin %s Tout %s\n",
		  SIMDTypeInfo<Tin>::name(), SIMDTypeInfo<Tout>::name()); }
    /* Second test */
    SIMDMask<Tin, SIMD_WIDTH> kzero;
    setcsr(csr0);
    res1=mask_cvts(src, kzero, a);
    if(!vectorsEqual(res1, src/*setzero<Tout, SIMD_WIDTH>()*/) || getcsr()!=csr0)
    { printf("Problem with %s for %s test 2, csr=%x, csr0=%x \na=", "cvts", __PRETTY_FUNCTION__, getcsr(), csr0); print("%e ", a); printf("\nres1="); print("%e ", res1); printf("\n");}
    setcsr(csr0);
    }

#define TEST_FUNCTION_LOAD_ALL_TYPES TEST_FUNCTION_LOAD_TYPE(SIMDByte, NATIVE_SIMD_WIDTH, "%3u ") TEST_FUNCTION_LOAD_TYPE(SIMDSignedByte, NATIVE_SIMD_WIDTH, "%3i ") TEST_FUNCTION_LOAD_TYPE(SIMDWord, NATIVE_SIMD_WIDTH, "%6u ") TEST_FUNCTION_LOAD_TYPE(SIMDShort, NATIVE_SIMD_WIDTH, "%6i ") TEST_FUNCTION_LOAD_TYPE(SIMDInt, NATIVE_SIMD_WIDTH, "%8i ") TEST_FUNCTION_LOAD_TYPE(SIMDFloat, NATIVE_SIMD_WIDTH, "%e ")

#define TEST_FUNCTION_STORE_ALL_TYPES TEST_FUNCTION_STORE_TYPE(SIMDByte, NATIVE_SIMD_WIDTH) TEST_FUNCTION_STORE_TYPE(SIMDSignedByte, NATIVE_SIMD_WIDTH) TEST_FUNCTION_STORE_TYPE(SIMDWord, NATIVE_SIMD_WIDTH) TEST_FUNCTION_STORE_TYPE(SIMDShort, NATIVE_SIMD_WIDTH) TEST_FUNCTION_STORE_TYPE(SIMDInt, NATIVE_SIMD_WIDTH) TEST_FUNCTION_STORE_TYPE(SIMDFloat, NATIVE_SIMD_WIDTH)

#define TEST_FUNCTION_ALL_INTS(NAME, OP) TEST_FUNCTION(NAME, OP, SIMDByte, NATIVE_SIMD_WIDTH) TEST_FUNCTION(NAME, OP, SIMDSignedByte, NATIVE_SIMD_WIDTH) TEST_FUNCTION(NAME, OP, SIMDWord, NATIVE_SIMD_WIDTH) TEST_FUNCTION(NAME, OP, SIMDShort, NATIVE_SIMD_WIDTH) TEST_FUNCTION(NAME, OP, SIMDInt, NATIVE_SIMD_WIDTH)

#define TEST_FUNCTION_ALL_TYPES(NAME, OP) TEST_FUNCTION_ALL_INTS(NAME, OP) TEST_FUNCTION(NAME, OP, SIMDFloat, NATIVE_SIMD_WIDTH)

#define TEST_FUNCTION_ALL_INTS_SOP(NAME, OP) TEST_FUNCTION_SOP(NAME, OP, SIMDByte, NATIVE_SIMD_WIDTH, "%u ") TEST_FUNCTION_SOP(NAME, OP, SIMDSignedByte, NATIVE_SIMD_WIDTH, "%i ") TEST_FUNCTION_SOP(NAME, OP, SIMDWord, NATIVE_SIMD_WIDTH, "%u ") TEST_FUNCTION_SOP(NAME, OP, SIMDShort, NATIVE_SIMD_WIDTH, "%i ") TEST_FUNCTION_SOP(NAME, OP, SIMDInt, NATIVE_SIMD_WIDTH, "%i ")

#define TEST_FUNCTION_ALL_TYPES_SOP(NAME, OP) TEST_FUNCTION_ALL_INTS_SOP(NAME, OP) TEST_FUNCTION_SOP(NAME, OP, SIMDFloat, NATIVE_SIMD_WIDTH, "%e ")

#define TEST_FUNCTION_ALL_TYPES_CMP(NAME, OP) TEST_FUNCTION_CMP(NAME, OP, SIMDByte, NATIVE_SIMD_WIDTH, "%u ") TEST_FUNCTION_CMP(NAME, OP, SIMDSignedByte, NATIVE_SIMD_WIDTH, "%i ") TEST_FUNCTION_CMP(NAME, OP, SIMDWord, NATIVE_SIMD_WIDTH, "%u ") TEST_FUNCTION_CMP(NAME, OP, SIMDShort, NATIVE_SIMD_WIDTH, "%i ") TEST_FUNCTION_CMP(NAME, OP, SIMDInt, NATIVE_SIMD_WIDTH, "%i ") TEST_FUNCTION_CMP(NAME, OP, SIMDFloat, NATIVE_SIMD_WIDTH, "%e ")

#define TEST_FUNCTION_ALL_TYPES_SHIFT(NAME, OP, COUNT) TEST_FUNCTION_SHIFT(NAME, OP, SIMDByte, NATIVE_SIMD_WIDTH, "%u ", COUNT) TEST_FUNCTION_SHIFT(NAME, OP, SIMDSignedByte, NATIVE_SIMD_WIDTH, "%i ", COUNT) TEST_FUNCTION_SHIFT(NAME, OP, SIMDWord, NATIVE_SIMD_WIDTH, "%u ", COUNT) TEST_FUNCTION_SHIFT(NAME, OP, SIMDShort, NATIVE_SIMD_WIDTH, "%i ", COUNT) TEST_FUNCTION_SHIFT(NAME, OP, SIMDInt, NATIVE_SIMD_WIDTH, "%i ", COUNT)

/*template <typename T, int SIMD_WIDTH>
void test_function(SIMDVec<T, SIMD_WIDTH> (*function) (const SIMDVec<T, SIMD_WIDTH> &src, const SIMDMask<T, SIMD_WIDTH> &k, const SIMDVec<T, SIMD_WIDTH> &a, const SIMDVec<T, SIMD_WIDTH> &b)) {
    _mm_getcsr()
}*/
...
void benchmark() {
    ...
    for(i=0;i<0x8fffffff;i++) {
        krand=random64();
        krand2=random_continuous(4);//TODO change parameter according to width of T
...
void test_mask_functions() {
    ...
    uinttest_t i;
    SIMDMask<T, SIMD_WIDTH> a_mask, b_mask, kand_mask, kandn_mask, kor_mask, kxor_mask, kxnor_mask, kadd_mask, knot_mask, kshiftli_mask, kshiftri_mask;
    uinttest_t ones=(SIMD_WIDTH/sizeof(T)==64?-1:((1ul<<(SIMD_WIDTH/sizeof(T)))-1)); //Conditional operator removes compiler warning
    uinttest_t a=ones&random64(), b=ones&random64();
    a_mask=a;
    b_mask=b;
    if(a!=((uinttest_t) a_mask) || b!=((uinttest_t) b_mask)) {
      printf("ERROR: mask conversion: a=%lx %lx, b=%lx %lx, ones=%lx, SIMD_WIDTH/sizeof(T)=%lu, %s\n", a, (uinttest_t) a_mask, b, (uinttest_t) b_mask, ones, SIMD_WIDTH/sizeof(T), __PRETTY_FUNCTION__);
    }
    for(i=0; i<64; i++) {
        if((a_mask[i])!=(((a>>i)&1)==1)) {
...
void testsuite() {
    ...
    TEST_FUNCTION_SHIFT(srai, dummy, SIMDInt, NATIVE_SIMD_WIDTH, "%i ", 3) //srai not defined for SIMDByte and SIMDSignedByte
    ...
    TEST_FUNCTION_MASK_TEST_ALL_(zeros, 0, SIMDSignedByte, NATIVE_SIMD_WIDTH, "%i ")
    ...
    TEST_FUNCTION_MASK_TEST_ALL_(ones, -1, SIMDSignedByte, NATIVE_SIMD_WIDTH, "%i ")
...
int main() {
    ...
    print("%u ", SIMDVec<SIMDShort, 64>(_mm512_maskz_sub_epi16(mask, a, b))); puts("");
    print("%u ", mask_ifelsezero(k, sub (a, b))); puts("");*/
    ...
    printf("%x \n", k);
    print("%i ", SIMDVec<SIMDSignedByte, NATIVE_SIMD_WIDTH>(_mm512_maskz_abs_epi8 (k, a))); puts(""); exit(0);*/
    unsigned int i;
    for(i=0; i<10000; i++) {
        testsuite();
    }
