...
{  
  ...
  void random_init_array(T *data, int length)
  {
    ...
    for (int i = 0; i < length; i++) {
      data[i] = T(float(rand())/float(RAND_MAX) *
...
void validate_vertical(int iterations)
{
  for(int i = 0; i < iterations; i++) {
    ...
    simd::verticalBitonicSort
      <LENGTH, simd::SortSlope::ASCENDING, T, SIMD_WIDTH>(copy_vertical);
    
    // sort with std::sort
    SortHelp::copy_array<T>(original, copy_stdsort, LENGTH);
    std::sort(&copy_stdsort[0], &copy_stdsort[LENGTH]);
    
    SortHelp::test_equal_array<T, SIMD_WIDTH>
      (copy_stdsort, copy_vertical, LENGTH);
...
void benchmark_vertical(int iterations)
{
  ...
  for(int i = 0; i < iterations; i++) {
    ...
    time = clock();
    simd::verticalBitonicSort
      <LENGTH, simd::SortSlope::ASCENDING, T, SIMD_WIDTH>(data);
    time = clock()-time;
